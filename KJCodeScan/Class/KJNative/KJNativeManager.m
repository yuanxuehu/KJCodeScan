...
//扫码结果
@property (nonatomic, strong) NSMutableArray<KJScanResult*> *arrayResult;
 ...
 *  扫码结果返回
 */
@property (nonatomic, copy)void (^blockScanResult)(NSArray<KJScanResult*> *array);

@property (nonatomic, copy) void (^blockvideoMaxScale)(CGFloat maxScale);

@end

@implementation KJNativeManager
...
- (instancetype)initWithPreView:(UIView *)preView
                  ...
                  videoMaxScale:(void(^)(CGFloat maxScale))blockvideoMaxScale
                        success:(void(^)(NSArray<KJScanResult*> *array))success
{
    if (self = [super init]) {
...
- (void)captureImage
{
    ...
    [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:stillImageConnection
                                                         completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
     {
        ...
        if (imageDataSampleBuffer)
        {
            ...
            for (KJScanResult* result in self.arrayResult) {
    ...
    NSDictionary *setDic = @{AVVideoCodecKey:AVVideoCodecTypeJPEG};
    AVCapturePhotoSettings *outputSettings = [AVCapturePhotoSettings photoSettingsWithFormat:setDic];
    [self.stillImageOutput capturePhotoWithSettings:outputSettings delegate:self];
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    for(AVMetadataObject *current in metadataObjects)
    {
        if ([current isKindOfClass:[AVMetadataMachineReadableCodeObject class]] )
        {
            ...
            {
                KJScanResult *result = [KJScanResult new];
...
#pragma mark --识别条码图片
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray<KJScanResult*> *array))block;
{
    if (!image) {
        block(nil);
        return;
    ...
    NSMutableArray<KJScanResult*> *mutableArray = [[NSMutableArray alloc]initWithCapacity:1];
    for (int index = 0; index < [features count]; index ++)
    {
        CIQRCodeFeature *feature = [features objectAtIndex:index];
        NSString *scannedResult = feature.messageString;
        KJScanResult *item = [[KJScanResult alloc]init];
